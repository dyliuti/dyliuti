...
def read_data_nmt():
    ...
    with zipfile.ZipFile(fname, 'r') as f:
        return f.read('jpn.txt').decode("utf-8")
    

# Defined in file: ./chapter_recurrent-neural-networks/machine-translation.md
def preprocess_nmt(text):
    text = text.replace('\u202f', ' ').replace('\xa0', ' ')
    no_space = lambda char, prev_char: (True if char in (',', '!', '.') and prev_char != ' ' else False)
...
def train_s2s_ch8(model, data_iter, lr, num_epochs, ctx):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        print('loss %.5f, %d tokens/sec on %s ' % (metric[0] / metric[1], metric[1] / timer.stop(), ctx))
