...
# accuracy:  98.14%; precision:  87.55%; recall:  84.55%; FB1:  86.02
#               LOC: precision:  87.85%; recall:  87.24%; FB1:  87.54  172800
#               ORG: precision:  83.32%; recall:  79.75%; FB1:  81.49  91700
# 模型参数
seg_dim = 20  #
word_dim = 100
lstm_dim = 100
...
for i in range(max_epoch):
    ...
    for batch in train_manager.iter_batch(shuffle=True):
        # chars: 20x23  segs: 20x23 tags: 20x23
        _, chars, segs, tags = batch
        feed_dict = {sentences_inputs: np.asarray(chars), seg_inputs: np.asarray(segs), targets: np.asarray(tags), dropout: p_dropout}
    ...
    for batch_ in dev_manager.iter_batch():
        ...
        lengths_dev, scores = sess.run([lengths, logits], feed_dict_dev)

        batch_paths = []
        for score, length in zip(scores, lengths_dev):
            score = score[:length]  # 左闭右开，
