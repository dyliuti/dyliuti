...
from bert_ import optimization
from bert_ import tokenization
from lstm_crf_layer import BLSTM_CRF
from bert_ import modeling
import tf_metrics
import pickle
...
bert_path = 'D:\Git\BERT\bert_finetune-master\chinese_L-12_H-768_A-12'
root_path = r'D:\Git\BERT\BERT_BiLSTM_CRF_NER'
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    """
    label_map = {}
    # 1表示从1开始对label进行index化,不包含0（填充的tag）
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    blstm_crf = BLSTM_CRF(embedded_chars=embedding, hidden_unit=lstm_size, cell_type=cell, num_layers=num_layers,
                          droupout_rate=droupout_rate, initializers=initializers, num_labels=num_labels,
    ...
    def model_fn(features, labels, mode, params):
        ...
        # +++
            ...
            def metric_fn(label_ids, logits, trans):
                ...
                f1 = tf_metrics.f1(label_ids, pred_ids, num_labels, [2, 3, 4, 5, 6, 7], weight)
...
#         raise ValueError("At least one of `do_train` or `do_eval` must be True.")
    ...
    model_fn = model_fn_builder(
        ...
        num_labels=len(label_list) + 1, # 1+6 + 1 + 2 = 10  10+1 = 11
    ...
    if do_eval:
        ...
        eval_steps = None
        if use_tpu:
            eval_steps = int(num_eval_size / eval_batch_size)
